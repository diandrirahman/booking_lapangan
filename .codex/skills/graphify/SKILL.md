...
When Graphify is used to inspect, plan, or modify code in a project:

- Keep every touched area clean and easy to read. Use descriptive names, small single-purpose functions or components, clear module boundaries, and consistent formatting.
- Avoid dense one-line implementations, duplicated business rules, unexplained magic values, dead code, and abstractions that do not reduce real complexity.
- Keep each domain rule in one reusable source of truth instead of reimplementing it differently across screens or flows.
- Refactor the code being touched when readability requires it, but do not expand the change into unrelated rewrites.
- Add comments only when they explain intent, constraints, or non-obvious tradeoffs; do not narrate code that is already clear.
- Before completion, run the project's relevant formatter or lint check, type-check, and tests for the touched scope. Report any remaining failure honestly.
